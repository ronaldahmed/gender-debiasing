...
class Preprocess(object):
    ...
    def build_wiki(self,max_vocab=20000):
        print("building vocab...")
        cnt = 0        
        data = nlp.data.WikiText2(segment="train", bos=None, eos=None, skip_empty=False)
        self.wc = Counter(data)
        self.wc[self.unk] = 1
        capped_vocab = [w for w,fq in self.wc.items() if fq>=5]
        self.idx2word = [self.unk] + capped_vocab
        self.word2idx = {self.idx2word[idx]: idx for idx, _ in enumerate(self.idx2word)}
        self.vocab = set(capped_vocab)

        print("Vocab size (>=5): ", len(self.vocab))
    ...
    def convert_wiki(self,):
        cnt = 0
        for split in ['train', 'val', 'test']:
            print("Converting "+ split)
            data = nlp.data.WikiText2(segment=split, bos=None, eos=None, flatten=False)
            print("\tnum. sents: ",len(data))
            task_data = []
            for sent in data:
                for i in range(len(sent)):
                    iword, owords = self.skipgram(sent, i)
                    task_data.append((self.word2idx.get(iword,0), [self.word2idx.get(oword,0) for oword in owords]))
            #
            print("\tnum. samples: ",len(task_data))
            with open(os.path.join(self.data_dir, split + '.dat'), 'wb') as outfile:
                pickle.dump(task_data, outfile)
            #
...
if __name__ == '__main__':
    ...
    preprocess.build_wiki()
    preprocess.convert_wiki()
